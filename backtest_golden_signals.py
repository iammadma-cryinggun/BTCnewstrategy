...
print(f"检测到黄金做多信号: {len(golden_long_signals)} 个")
...
# 出场策略
HOLD_MAX_PERIODS = 10     # 最多持有10个周期（40小时）
...
print(f"  最大持仓周期: {HOLD_MAX_PERIODS} ({HOLD_MAX_PERIODS*4}小时)")
...
if len(trades_df) == 0:
    ...
    trades_df[output_cols].to_csv('黄金信号回测结果.csv', index=False, encoding='utf-8-sig')
    print(f"\n交易记录已保存: 黄金信号回测结果.csv")
