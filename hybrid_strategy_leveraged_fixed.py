...
策略说明：
【基础逻辑】严格复现简单策略（70%胜率）
- 入场：信号切换到LONG_MODE（NO_TRADE→LONG 或 SHORT→LONG）
- 离场：高点或信号切换出LONG_MODE
- 滞后确认：2根K线
...
for i in range(len(df)):
    ...
    if current_position == 'NONE':
        ...
        # 修复：必须当前信号是LONG_MODE，且之前不是LONG_MODE
...
if len(trades) == 0:
    ...
    # 保存结果
    trades_df.to_csv('混合策略V2_5倍杠杆_修复版_回测结果.csv', index=False, encoding='utf-8-sig')
    print(f"\n交易记录已保存: 混合策略V2_5倍杠杆_修复版_回测结果.csv")
