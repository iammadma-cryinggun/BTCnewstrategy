...
print("=" * 80)
print("所有4种信号类型的完整黄金信号演变分析（使用正确数据）")
print("=" * 80)
...
# 保存完整数据
df_complete.to_csv('complete_analysis_v5_correct.csv', index=False, encoding='utf-8-sig')
print(f"\n已保存: complete_analysis_v5_correct.csv")

# ==================== 分析函数 ====================
def analyze_signal_type(df, signal_type):
    """分析某种信号类型的统计特征"""
    ...
    # 统计显著性特征
    print(f"\n显著特征（p<0.05）:")
    features = ['开仓时加速度', '开仓时量能比率', '平仓时张力', '平仓时加速度', '平仓时量能比率']
...
analyze_signal_type(df_complete, 'BEARISH_SINGULARITY')
analyze_signal_type(df_complete, 'BULLISH_SINGULARITY')
analyze_signal_type(df_complete, 'HIGH_OSCILLATION')
analyze_signal_type(df_complete, 'LOW_OSCILLATION')
