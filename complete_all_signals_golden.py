...
包含：
1. BEARISH_SINGULARITY（看跌奇点）
2. BULLISH_SINGULARITY（看涨奇点）
3. HIGH_OSCILLATION（高位震荡）
4. LOW_OSCILLATION（低位震荡）
"""
...
print("=" * 80)
print("所有4种信号类型的完整黄金信号演变分析")
print("=" * 80)

# ==================== 读取数据 ====================
df_signals = pd.read_csv('step1_entry_signals_correct_accel.csv', encoding='utf-8-sig')
...
print(f"信号类型分布:")
print(df_complete['信号类型'].value_counts())

# ==================== 分析函数 ====================
def analyze_signal_type(df, signal_type, direction_name):
    """分析某种信号类型的完整演变"""
    ...
    if len(df_type) == 0:
        print(f"\n{'=' * 80}")
        print(f"{signal_type} - 没有数据")
        print(f"{'=' * 80}")
        return

    print(f"\n{'=' * 80}")
    print(f"{signal_type}（{direction_name}）完整演变分析")
    print(f"{'=' * 80}")
    ...
    for col in ['信号前张力', '信号前加速度', '信号前量能比率', '信号前DXY燃料']:
        ...
        print(f"{col}: 好={good_mean:.4f}, 坏={bad_mean:.4f}, t={t_stat:.3f}, p={p_val:.4f} {sig}")
    ...
    for col in ['开仓时张力', '开仓时加速度', '开仓时量能比率', '开仓时DXY燃料']:
        ...
        print(f"{col}: 好={good_mean:.4f}+/-{good_std:.4f}, 坏={bad_mean:.4f}, d={cohens_d:.3f}({effect}), p={p_val:.4f} {sig}")
        ...
        try:
            ...
            if optimal_youden > 0.2:
                ...
                if len(above) > 0 and len(below) > 0:
                    ...
                    print(f"\n{feature}: 阈值={optimal_threshold:.4f}, Youden={optimal_youden:.4f}({discrim}), 以上好机会率={above_rate:.1f}%, 以下好机会率={below_rate:.1f}%")
        except:
            pass

# ==================== 分析所有4种信号类型 ====================
print("\n" + "=" * 80)
print("开始系统性分析所有4种信号类型...")
print("=" * 80)

analyze_signal_type(df_complete, 'BEARISH_SINGULARITY', '做空')
analyze_signal_type(df_complete, 'BULLISH_SINGULARITY', '做多')
analyze_signal_type(df_complete, 'HIGH_OSCILLATION', '做空')
analyze_signal_type(df_complete, 'LOW_OSCILLATION', '做多')
...
# 保存数据
df_complete.to_csv('complete_all_signals_analysis.csv', index=False, encoding='utf-8-sig')
print(f"\n已保存: complete_all_signals_analysis.csv")
