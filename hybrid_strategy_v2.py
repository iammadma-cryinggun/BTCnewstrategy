...
# 黑天鹅交易参数
BS_POSITION_PCT = 0.80
BS_STOP_LOSS_PCT = 0.02

print(f"回测参数：")
print(f"- 初始资金: ${INITIAL_CAPITAL:,.2f}")
print(f"- 正常仓位: {NORMAL_POSITION_PCT*100}%, 止损: {NORMAL_STOP_LOSS_PCT*100}%")
print(f"- 黑天鹅仓位: {BS_POSITION_PCT*100}%, 止损: {BS_STOP_LOSS_PCT*100}%")
...
entry_signal_idx = None  # 记录入场时的信号位置
    ...
    if current_position == 'NONE' and is_new_signal:
        ...
        if signal_mode == 'LONG_MODE':
            ...
            if i + order < len(df):
                ...
                entry_signal_idx = i  # 记录原始信号位置

                # 检查原始信号是否是黑天鹅
                if df.loc[entry_signal_idx, '是黑天鹅']:
                    position_size = capital * BS_POSITION_PCT
                    stop_loss_pct = BS_STOP_LOSS_PCT
                    is_black_swan_trade = True
                    current_position = 'LONG'
                else:
                    position_size = capital * NORMAL_POSITION_PCT
...
if len(trades) == 0:
    ...
    # 保存结果
    trades_df.to_csv('混合策略V2_回测结果.csv', index=False, encoding='utf-8-sig')
    print(f"\n交易记录已保存: 混合策略V2_回测结果.csv")
