# -*- coding: utf-8 -*-
"""
V7.0.5黄金信号完整统计学分析 - 含DXY燃料
==========================================

基于完整验证5逻辑（含DXY燃料）的统计学分析：
- p值显著性检验（t检验，p<0.05）
- Cohen's d效应量（>0.5中等，>0.8大，>1.2超大）
- Youden Index（最优判别阈值，>0.8优秀）
- 95%置信区间

数据：step2_3_analysis_results_with_dxy.csv（2025-07-25至2026-01-20）
"""
...
print("=" * 80)
print("V7.0.5黄金信号完整统计学分析 - 含DXY燃料")
print("=" * 80)

# ==================== 读取数据 ====================
df = pd.read_csv('step2_3_analysis_results_with_dxy.csv', encoding='utf-8')

# 转换数据类型
df['信号时间'] = pd.to_datetime(df['信号时间'])
for col in ['张力', '首次开仓盈亏%', '最优开仓盈亏%', '最优平仓盈亏%', '好机会盈亏%', '价格优势%', 'DXY燃料']:
...
if len(df_short) > 0:
    df_short['是好机会_binary'] = df_short['最优平仓盈亏%'] > 0
    ...
    # 价格优势%
    good_vals = good_short['价格优势%']
    bad_vals = bad_short['价格优势%']
    ...
    if len(good_vals) > 0 and len(bad_vals) > 0:
        ...
        # Youden Index
        try:
            threshold, youden = find_youden_threshold(df_short['价格优势%'], good_short, df_short)
    ...
    # DXY燃料
    if 'DXY燃料' in df_short.columns:
        good_vals = good_short['DXY燃料']
        bad_vals = bad_short['DXY燃料']
...
if len(df_long) > 0:
    df_long['是好机会_binary'] = df_long['最优平仓盈亏%'] > 0
    ...
    # 价格优势%
    good_vals = good_long['价格优势%']
    bad_vals = bad_long['价格优势%']
    ...
    if len(good_vals) > 0 and len(bad_vals) > 0:
        ...
        # Youden Index
        try:
            threshold, youden = find_youden_threshold(df_long['价格优势%'], good_long, df_long)
            print(f"  Youden阈值: {threshold:.4f}, Youden指数: {youden:.4f}",
                  f"{'优秀' if youden > 0.8 else '良好' if youden > 0.5 else '一般'}")
        except:
            pass
    ...
    # DXY燃料
    if 'DXY燃料' in df_long.columns:
        good_vals = good_long['DXY燃料']
        bad_vals = bad_long['DXY燃料']
...
print("\n" + "=" * 80)
print("[OK] V7.0.5黄金信号统计学分析完成（含DXY燃料）")
print("=" * 80)
