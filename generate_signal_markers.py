...
for i in range(len(df)):
    ...
    if current_position == 'NONE':
        ...
        # 必须当前信号是LONG_MODE，且之前不是LONG_MODE
        if signal_mode == 'LONG_MODE' and prev_signal_mode != 'LONG_MODE':
            ...
            if i + order < len(df):
                ...
                if df.loc[entry_signal_idx, '是黑天鹅']:
                    ...
                    df.loc[entry_idx:, '策略状态'] = '持仓'
                    df.loc[entry_idx:, '持仓类型'] = 'LONG黑天鹅'
                else:
    ...
    if current_position == 'LONG':
        ...
        # 如果当前行没有动作，检查下一行是否有动作（避免覆盖）
        if pd.isna(df.loc[i+1, '信号动作']) or df.loc[i+1, '信号动作'] == '':
            if action:
                df.loc[i, '信号动作'] = action
...
# 只输出有动作或持仓的行
# 但为了核对，我们输出所有行，标记出关键位置

print("\n正在保存完整数据表...")

# 保存完整版本
output_df.to_csv('带信号标记_完整数据.csv', index=False, encoding='utf-8-sig')

print("[OK] 已保存: 带信号标记_完整数据.csv")
...
action_df.to_csv('带信号标记_关键行.csv', index=False, encoding='utf-8-sig')

print("[OK] 已保存: 带信号标记_关键行.csv")
...
if len(all_actions) > 0:
    print(f"\n{'时间':<20} {'收盘价':<12} {'信号类型':<20} {'黑天鹅':<10} {'动作':<30}")
    print("-"*120)
...
print("\n" + "="*120)
print("[OK] 完成！请打开 '带信号标记_完整数据.csv' 核对")
print("="*120)
