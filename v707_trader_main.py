...
class V707TraderConfig:
    ...
    def __init__(self):
        ...
        self.TAKE_PROFIT_PCT = 0.05  # 固定止盈 +5%
        self.STOP_LOSS_PCT = -0.025  # 固定止损 -2.5%
...
class PhysicsSignalCalculator:
    ...
    def diagnose_regime(self, tension, acceleration, dxy_fuel=0.0):
        ...
        signal_type: 'BEARISH_SINGULARITY' | 'BULLISH_SINGULARITY' | 'OSCILLATION' | 'HIGH_OSCILLATION' | 'LOW_OSCILLATION' | None

        ⚠️ DXY燃料说明：
        - dxy_fuel=0.0（默认）：不使用DXY增强，置信度0.6~0.8
        - dxy_fuel>0：使用DXY增强，强奇点信号置信度可达0.9~0.95
        """
        confidence = 0.0
        signal_type = None
        description = "无信号"
